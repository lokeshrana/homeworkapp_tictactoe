...
export default class Board extends Model {
  ...
  public async boards(limit: number, after: any, orderBy: any, filter: any) {
    const queryBuilder = Board.query().withGraphFetched(eager).orderBy('id', 'desc');
    ...
    if (filter) {
      if (has(filter, 'userId') && filter.userId !== '') {
        queryBuilder.where(function () {
          this.where('board.user_1_id', filter.userId).orWhere('board.user_2_id', filter.userId);
        });
      }
    ...
    if (limit && after) {
      res = camelizeKeys(await queryBuilder.limit(limit).offset(after).groupBy('board.id'));
    } else if (limit && !after) {
  ...
  public async board(id: number) {
    let res = {};
    res = camelizeKeys(await Board.query().findById(id).withGraphFetched(eager).orderBy('id', 'desc'));
    return res;
  }
  ...
  public async getBoardByBothUserIds(userId1: number, userId2: number) {
    const res = await Board.query()
      .where(function () {
        this.where('board.user_1_id', userId1).andWhere('board.user_2_id', userId2);
      })
      .orWhere(function () {
        this.where('board.user_1_id', userId2).andWhere('board.user_2_id', userId1);
      })
